...
#[test]
fn global_struct_array_with_double_singleton_inner_dims_and_ptr_field_initializes_first_scalar() {
    let stamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock before unix epoch")
        .as_nanos();
    let tmp_dir = std::env::temp_dir().join(format!(
        "ccc_regress_struct_array_double_singleton_inner_dim_ptr_global_init_{}_{}",
        std::process::id(),
        stamp
    ));
