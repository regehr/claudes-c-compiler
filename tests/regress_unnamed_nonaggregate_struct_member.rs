...
#[test]
fn unnamed_nonaggregate_struct_decl_must_not_create_initializer_slot() {
    let stamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock before unix epoch")
        .as_nanos();
    let tmp_dir = std::env::temp_dir().join(format!(
        "ccc_regress_unnamed_nonaggregate_struct_member_{}_{}",
        std::process::id(),
        stamp
    ));
...
"#;
    ...
    assert_eq!(run.stdout, b"checksum = 3\n");
